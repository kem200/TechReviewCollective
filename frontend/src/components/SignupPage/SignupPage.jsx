...
import { signup, setErrors } from '../../store/session'; // Import setErrors action creator
...
const SignupPage = () => {
  ...
  const errors = useSelector((state) => state.session.errors); // Get errors from Redux store

  const handleSubmit = async (e) => {
    e.preventDefault();
    if (password !== confirmPassword) {
      dispatch(setErrors(['Passwords do not match'])); // Correctly dispatch setErrors
      return;
    }
  ...
  return (
    <div className="SignupPage-Main">
      ...
      <form onSubmit={handleSubmit}>
        {errors && errors.length > 0 && ( // Display errors from Redux store
